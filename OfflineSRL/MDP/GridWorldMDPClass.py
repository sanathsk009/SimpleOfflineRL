...
def _error_check(state, action):
    ...
    if action not in GridWorldMDP.ACTIONS:
        raise ValueError(
            "(simple_rl) GridWorldError: the action provided (" + str(action) + ") was invalid in state: " + str(
                state) + ".")

    if not isinstance(state, GridWorldState):
        raise ValueError(
            "(simple_rl) GridWorldError: the given state (" + str(state) + ") was not of the correct class.")
...
def make_grid_world_from_file(file_name, randomize=False, num_goals=1, name=None, goal_num=None, slip_prob=0.0):
    ...
    return GridWorldMDP(width=num_cols, height=num_rows, init_loc=(agent_x, agent_y), goal_locs=goal_locs,
